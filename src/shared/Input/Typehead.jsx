...
import './Input.scss';
import { getReqCities } from '../../core/services/axios';
...
// Si el input tiene 3 o más caracteres, muestra el panel de opciones con las opciones filtradas
         ...
         const filter = cities.filter(city =>
            city.title.toLowerCase().includes($event.target.value.toLowerCase())
         );
         setItemsFiltered(
            filter.length
      ...
      <div className="input" ref={typeheadRef}>
         ...
         {searching ? (
            <div className="panelContainer">
               <PanelOptions items={cities} selectOption={handleChange} />
            </div>
         ) : null}
